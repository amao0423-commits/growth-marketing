...
type SubmitResult = { id: number; editToken: string };
...
export default function SubmitPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setResult({ id: data.id, editToken: data.editToken });
    } catch {
  ...
  if (result) {
    return (
      <div className="ap-wrap" style={{ maxWidth: 760, padding: "40px 20px 80px" }}>
        <section className="ap-post" style={{ background: "var(--ok-bg, #D3F0E4)", color: "var(--ok-fg, #1F6B52)", borderRadius: 14, padding: 28, marginBottom: 20 }}>
        ...
        <section className="sec" style={{ marginBottom: 16 }}>
          <h2>記事の紹介・取り下げについて</h2>
          <p style={{ fontSize: 13, lineHeight: 1.85 }}>
            公開日から14日以内に、記事を紹介したページ（自社サイトのお知らせ欄・SNS投稿など）のURLを{" "}
            <a href="mailto:a-ando@hotseller.jp">a-ando@hotseller.jp</a> までメールでお送りください。
            内容の修正・取り下げをご希望の場合も同じ宛先までご連絡ください。その際、下記の編集用トークンをお伝えください。
          </p>
          <p className="mono" style={{ background: "#F6F6F8", borderRadius: 8, padding: "10px 14px", wordBreak: "break-all" }}>
            編集トークン: {result.editToken}
          </p>
        </section>
      </div>
    );
    ...
    <form className="ap-wrap" style={{ maxWidth: 760, padding: "40px 20px 80px" }} onSubmit={handleSubmit}>
      ...
      <section className="sec" style={{ marginBottom: 16, border: "1px solid #E7E7EC", borderRadius: 10, padding: 22 }}>
        ...
        <label style={{ display: "flex", gap: 10, alignItems: "flex-start", fontSize: 13, marginBottom: 12, cursor: "pointer" }}>
          ...
          <span>掲載後は、自社サイトのお知らせやSNSなどで記事を紹介し、そのページのURLを14日以内にメールで提出します。提出がない場合、記事は取り下げられることがあります。</span>
        </label>
